...
def generate_release_notes_with_ai(
    ...
    prompt = f"""You are writing release notes for "Spokn" - a macOS menu bar text-to-speech app with karaoke-style word highlighting.
...
1. **Structure**: Use sections like "Features", "Bug Fixes", "Improvements", "Maintenance" (only if applicable)
...
4. **Content**:
   ...
   - Omit version bumps, merge commits

Example style:
- ✨ Add **karaoke mode** with word-by-word highlighting
...
Generate ONLY the release notes, starting with the first section header."""
    ...
    response = requests.post(
        ...
        headers={
            ...
            "X-Title": "Spokn Release Notes Generator",
        },
        json={
            "model": model,
